  ...
  <div class="space-y-4">
    ...
    >
      <div>
        ...
        >
          Visual Check Monitoring
        </h3>
        <p class="text-[10px] lg:text-xs text-slate-500 dark:text-slate-400">
          List of PCBs processed through Visual Check station.
        </p>
      </div>
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <div
          class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg flex overflow-hidden whitespace-nowrap text-[10px] lg:text-xs transition-colors"
        >
          <button
            @click="setJudgement('')"
            :class="
              params.judgement === ''
                ? 'bg-slate-100 dark:bg-slate-700 font-bold dark:text-white'
                : 'bg-slate-50 dark:bg-slate-800/50 font-medium'
            "
            class="px-3 py-1.5 text-slate-600 dark:text-slate-300 border-r border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
          >
            All
          </button>
          <button
            @click="setJudgement('0')"
            :class="
              params.judgement === '0'
                ? 'bg-slate-100 dark:bg-slate-700 font-bold dark:text-white'
                : 'bg-slate-50 dark:bg-slate-800/50 font-medium'
            "
            class="px-3 py-1.5 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 border-r border-slate-200 dark:border-slate-700 transition-colors"
          >
            OK
          </button>
          <button
            @click="setJudgement('1')"
            :class="
              params.judgement === '1'
                ? 'bg-slate-100 dark:bg-slate-700 font-bold dark:text-white'
                : 'bg-slate-50 dark:bg-slate-800/50 font-medium'
            "
            class="px-3 py-1.5 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 transition-colors"
          >
            NG
          </button>
        </div>
    ...
    >
      <div class="overflow-x-auto min-h-[300px]">
        <table class="w-full text-left text-xs whitespace-nowrap">
          <thead>
            <tr
              class="bg-slate-50/50 dark:bg-slate-800/50 text-slate-400 dark:text-slate-500 uppercase tracking-widest text-[9px] lg:text-[10px]"
            >
          ...
          <tbody class="divide-y divide-slate-50 dark:divide-slate-700/50">
            <template v-if="isLoading">
              <tr v-for="i in 5" :key="'skeleton-' + i" class="animate-pulse">
                ...
                <td class="px-4 py-3">
                  <div
                    class="h-3 bg-slate-200 dark:bg-slate-700 rounded w-32"
                  ></div>
                </td>
                <td class="px-4 py-3 text-center">
                  <div
                    class="h-4 bg-slate-200 dark:bg-slate-700 rounded-full w-12 mx-auto"
                  ></div>
                </td>
                <td class="px-4 py-3 text-right">
                  <div
                    class="h-3 bg-slate-200 dark:bg-slate-700 rounded w-16 ml-auto"
                  ></div>
                </td>
              </tr>
            </template>
            <template v-else-if="records.length > 0">
              ...
              >
                <td
                  class="px-4 py-2.5 font-bold text-slate-700 dark:text-slate-200"
                >
                  {{ pcb.qrCode }}
                </td>
                <td
                  class="px-4 py-2.5 text-slate-500 dark:text-slate-400 font-medium text-[10px] lg:text-xs"
                >
                  {{
                    pcb.createdAt
                      ? new Date(pcb.createdAt).toLocaleString()
                      : "-"
                  }}
                </td>
                <td class="px-4 py-2.5 text-center">
                  <span
                    ...
                    "
                    class="inline-flex items-center justify-center rounded-full px-2 py-0.5 text-[9px] lg:text-[10px] font-black uppercase tracking-tighter"
                  >
                    {{ pcb.judgement }}
                  </span>
                </td>
              ...
              <tr>
                <td colspan="4" class="px-4 py-16 text-center">
                  <div class="flex flex-col items-center justify-center">
                    <div
                      class="bg-red-50 dark:bg-red-900/30 w-16 h-16 rounded-full flex items-center justify-center mb-4 border border-red-100 dark:border-red-800/50"
                    >
                      <TriangleAlert
                    ...
                    <p
                      class="text-xs text-slate-500 dark:text-slate-400 mt-1 max-w-sm mx-auto leading-relaxed"
                    >
                      {{
                        error?.response?.data?.message ||
                        error?.message ||
                        "An unexpected error occurred while fetching camera checks."
                      }}
                    </p>
                    <button
                      @click="refetch()"
                      class="mt-5 px-5 py-2 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 shadow-sm border border-slate-200 dark:border-slate-700 text-xs font-semibold rounded-lg transition-colors flex items-center gap-2"
                    >
                      <svg
            ...
            <template v-else-if="!isLoading && !isError">
              <tr>
                <td colspan="4" class="px-4 py-16 text-center">
                  <div class="flex flex-col items-center justify-center">
                    <div
                      class="bg-slate-50 dark:bg-slate-800/50 w-16 h-16 rounded-full flex items-center justify-center mb-4 border border-slate-100 dark:border-slate-700/50"
                    >
                      <SearchX
                    ...
                    <p
                      class="text-xs text-slate-500 dark:text-slate-400 mt-1 max-w-sm mx-auto leading-relaxed"
                    >
                      We couldn't find any data for the selected period or
                      search criteria.
                    </p>
                    <button
                      v-if="searchRef"
