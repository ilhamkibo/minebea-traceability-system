...
import { TriangleAlert, Pencil, Trash2, Plus, X, Users } from 'lucide-vue-next'
...
const paginationMeta = computed(() => {
  return operatorsResponse.value?.pagination || null
})
...
const handleSubmit = async () => {
  ...
  if (!form.name || !form.pin) {
    formError.value = 'Name and PIN are required.'
    return
  }
...
const confirmDelete = async () => {
  ...
  if (!deletePin.value) {
    deleteError.value = 'PIN is required.'
    return
  }
  try {
    ...
    if (err instanceof AxiosError) {
      ...
      deleteError.value = 'Failed to delete operator.'
    }
  }
}
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg flex overflow-hidden whitespace-nowrap text-[10px] lg:text-xs transition-colors">
          <button @click="setRole('')" :class="params.role === '' ? 'bg-slate-100 dark:bg-slate-700 font-bold dark:text-white' : 'bg-slate-50 dark:bg-slate-800/50 font-medium'" class="px-3 py-1.5 text-slate-600 dark:text-slate-300 border-r border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">All</button>
          <button @click="setRole('Admin')" :class="params.role === 'Admin' ? 'bg-slate-100 dark:bg-slate-700 font-bold dark:text-white' : 'bg-slate-50 dark:bg-slate-800/50 font-medium'" class="px-3 py-1.5 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 border-r border-slate-200 dark:border-slate-700 transition-colors">Admin</button>
          <button @click="setRole('Operator')" :class="params.role === 'Operator' ? 'bg-slate-100 dark:bg-slate-700 font-bold dark:text-white' : 'bg-slate-50 dark:bg-slate-800/50 font-medium'" class="px-3 py-1.5 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 transition-colors">Operator</button>
        </div>
        ...
        <table class="w-full text-left text-xs whitespace-nowrap">
          ...
          <tbody class="divide-y divide-slate-50 dark:divide-slate-700/50">
            ...
            <template v-else-if="!isLoading && !isError">
              <tr>
                <td colspan="6" class="px-4 py-10 text-center text-slate-400 font-medium">No operators found.</td>
              </tr>
            </template>
          </tbody>
        </table>
        ...
        <div v-if="showModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div class="relative bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-100 dark:border-slate-700 w-full max-w-md overflow-hidden transition-colors">
            ...
            <div class="p-5 space-y-4">
              <div v-if="formError" class="p-3 bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800/30 rounded-lg text-xs text-red-600 dark:text-red-400 font-medium">
                {{ formError }}
              </div>
        ...
        <div v-if="showDeleteModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div class="relative bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-100 dark:border-slate-700 w-full max-w-sm overflow-hidden transition-colors">
            ...
            <div class="p-5 space-y-4">
              ...
              <div v-if="deleteError" class="p-3 bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800/30 rounded-lg text-xs text-red-600 dark:text-red-400 font-medium">
                {{ deleteError }}
              </div>
