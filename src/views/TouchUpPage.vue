...
import { useTouchUps } from "@/hooks/useTouchUp";
...
const records = computed(() => {
  if (touchUps.value?.data && Array.isArray(touchUps.value.data)) {
    return touchUps.value.data;
  }
  if (Array.isArray(touchUps.value)) {
    return touchUps.value;
  }
  return [];
});

const paginationMeta = computed(() => {
  return touchUps.value?.pagination || null;
});
</script>
    ...
    >
      <div>
        ...
        <p class="text-[10px] lg:text-xs text-slate-500 dark:text-slate-400">
          List of PCBs processed through Touch Up station.
        </p>
      </div>
                  ...
                  <div class="flex flex-col items-center justify-center">
                    ...
                    >
                      {{
                        ...
                        error?.message ||
                        "An unexpected error occurred while fetching touch up data."
                      }}
                    </p>
                    <button
                      @click="refetch()"
