...
<template>
  <div class="space-y-4">
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
      <div>
        <h3 class="text-lg lg:text-xl font-bold text-slate-800 dark:text-slate-100">{{ processName }} Monitoring</h3>
        <p class="text-[10px] lg:text-xs text-slate-500 dark:text-slate-400">List of PCBs processed through {{ processName }} station.</p>
      </div>
      <div class="flex space-x-2 overflow-x-auto">
        <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg flex overflow-hidden whitespace-nowrap text-[10px] lg:text-xs transition-colors">
           <button class="px-3 py-1.5 font-medium bg-slate-50 dark:bg-slate-700 text-slate-600 dark:text-slate-200 border-r border-slate-200 dark:border-slate-700">All</button>
           <button class="px-3 py-1.5 font-medium hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 border-r border-slate-200 dark:border-slate-700 transition-colors">OK</button>
           <button class="px-3 py-1.5 font-medium hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 transition-colors">NG</button>
        </div>
      </div>
    </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left text-xs whitespace-nowrap">
          ...
          <tbody class="divide-y divide-slate-50 dark:divide-slate-700/50">
            <tr v-for="pcb in filteredRecords" :key="pcb.id" class="hover:bg-slate-50/50 dark:hover:bg-slate-700/50 transition-colors">
              <td class="px-4 py-2 font-bold text-slate-700 dark:text-slate-200">{{ pcb.id }}</td>
              <td class="px-4 py-2 text-slate-500 dark:text-slate-400 font-medium text-[10px] lg:text-xs">{{ new Date(pcb.stepTime).toLocaleString() }}</td>
