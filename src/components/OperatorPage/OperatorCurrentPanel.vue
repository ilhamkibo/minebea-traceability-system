...
import { useCurrentOperators, useOperatorsList, useUpdateAssignments } from '@/hooks/useOperators'
...
const { data: currentOperatorsResponse, isLoading: isCurrentLoading } = useCurrentOperators()
...
const { data: dropdownOperatorsResponse, isLoading: isDropdownLoading } = useOperatorsList(params)
...
const assignedOperatorCache = reactive<Map<string, { id: number; name: string }>>(new Map())

const getOperatorAtPosition = (positionKey: string) => {
  return currentOperators.value.find(op => op.workPosition.includes(positionKey))
}
...
const mergedDropdownOperators = computed(() => {
  ...
  const assignedIds = new Set(assignmentForm.value.map(r => r.operatorId).filter(Boolean))
  ...
  for (const id of assignedIds) {
    if (!serverIds.has(id)) {
      ...
      if (cached) {
        extraOps.push({ id: cached.id, name: cached.name, assigned: true, workPosition: [] })
      }
    }
  }
...
const handleAssignmentSubmit = async () => {
  ...
  try {
    await assignmentMutation.mutateAsync({ data: payload, pin: assignmentPin.value })
...
const selectOperator = (index: number, operator: Operator) => {
  ...
  assignedOperatorCache.set(operator.id.toString(), { id: operator.id, name: operator.name })
  // Sync Touch Up ↔ Final Inspect
  if (SYNCED_INDICES.includes(index)) {
    const syncTarget = SYNCED_INDICES.find(i => i !== index)!
    assignmentForm.value[syncTarget].operatorId = operator.id.toString()
  }
  activeDropdownIndex.value = null
  dropdownSearchInput.value = ''
}
...
const clearOperator = (index: number) => {
  ...
  if (SYNCED_INDICES.includes(index)) {
    const syncTarget = SYNCED_INDICES.find(i => i !== index)!
    assignmentForm.value[syncTarget].operatorId = ''
  }
}
  ...
  <div>
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden transition-colors">
      <div class="px-4 py-3 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
          <div v-if="isCurrentLoading" class="flex items-center gap-1.5 text-[10px] text-slate-400">
          ...
          <button @click="openAssignmentModal" class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors flex items-center gap-1.5 shadow-sm">
      ...
      <div class="p-4 grid grid-cols-2 md:grid-cols-4 gap-3">
        <div v-for="position in [
          ...
          <template v-else>
            <div v-if="getOperatorAtPosition(position.key)" class="flex items-center gap-2">
              <div class="w-7 h-7 rounded-full bg-brand-accent/10 dark:bg-brand-accent/20 flex items-center justify-center shrink-0">
                <Users class="w-3.5 h-3.5 text-brand-accent" />
              </div>
              <div>
                <div class="text-xs font-bold text-slate-700 dark:text-slate-200">{{ getOperatorAtPosition(position.key)?.name }}</div>
                <div class="text-[9px] text-green-500 font-medium">● Active</div>
              </div>
            </div>
            <div v-else class="flex items-center gap-2">
              ...
              <div class="text-[10px] text-slate-400 dark:text-slate-500 font-medium">Unassigned</div>
            </div>
          </template>
        </div>
      </div>
    ...
    <Teleport to="body">
      <Transition name="fade">
        <div v-if="showAssignmentModal" class="fixed inset-0 z-100 flex items-center justify-center p-4">
          <div class="absolute inset-0 bg-brand-dark/50 dark:bg-black/60 backdrop-blur-sm" @click="closeAssignmentModal"></div>
          <div class="relative bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-100 dark:border-slate-700 w-full max-w-lg overflow-hidden transition-colors">
            <!-- Header -->
            <div class="px-5 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
              ...
              <button @click="closeAssignmentModal" class="p-1 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors">
                <X class="w-4 h-4" />
              </button>
            </div>
            ...
            <div class="p-5 space-y-4 max-h-[60vh] overflow-y-auto">
              <div v-for="(row, index) in assignmentForm" :key="index" class="pb-4 border-b border-slate-100 dark:border-slate-700 last:border-b-0">
                <label class="block text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">
                  {{ row.label }}
                </label>
                ...
                <div class="relative">
                  ...
                  >
                    <span :class="!row.operatorId ? 'text-slate-400' : ''">{{ selectedOperatorName(index) }}</span>
                    <svg class="w-3 h-3 text-slate-400 transition-transform" :class="activeDropdownIndex === index ? 'rotate-180' : ''" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <!-- Dropdown menu -->
                  <Transition name="fade">
                    <div v-if="activeDropdownIndex === index" class="absolute z-50 mt-1 w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg overflow-hidden">
                      ...
                      <div class="max-h-48 overflow-y-auto">
                        ...
                        <div v-if="isDropdownLoading" class="px-3 py-4 text-center text-xs text-slate-400 flex items-center justify-center gap-1.5">
                          <svg class="animate-spin h-3 w-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
                          </svg>
                        ...
                        <div v-else-if="mergedDropdownOperators.length === 0" class="px-3 py-4 text-center text-xs text-slate-400">
                          No operators found.
                        </div>
                        <button
                          v-for="(op,opIndex) in mergedDropdownOperators.slice(0, 10)"
                          :key="op.id"
                          type="button"
                          ...
                          :class="{ 'bg-brand-accent/5 dark:bg-brand-accent/10 font-semibold': row.operatorId === op.id.toString() }"
                        >
                          ...
                          <span>{{ op.name }}</span>
                          <span v-if="op.assigned && op.workPosition && op.workPosition.length > 0" class="ml-auto text-[9px] text-green-500 font-medium whitespace-nowrap">{{ op.workPosition.join(', ') }}</span>
                        </button>
                      </div>
                    </div>
                  </Transition>
              ...
              <div class="pt-2">
                ...
                <input v-model="assignmentPin" type="password" class="w-full text-xs px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-accent/20 transition-colors" placeholder="Enter PIN to confirm assignment" @keyup.enter="handleAssignmentSubmit" />
              </div>
            </div>

            <!-- Footer -->
            <div class="px-5 py-4 border-t border-slate-100 dark:border-slate-700 flex items-center justify-end gap-2">
              <button @click="closeAssignmentModal" class="px-4 py-2 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 rounded-lg text-xs font-semibold hover:bg-slate-50 dark:hover:bg-slate-600 transition-colors">
                Cancel
              </button>
              <button @click="handleAssignmentSubmit" :disabled="assignmentMutation.isPending.value" class="px-4 py-2 bg-brand-accent hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg text-xs font-bold transition-colors flex items-center gap-1.5">
